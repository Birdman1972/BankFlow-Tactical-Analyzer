...
graph TB
    subgraph App["App.svelte"]
        Header["Header<br/>標題區"]

        subgraph Main["主要內容區"]
            subgraph FileSection["檔案輸入區"]
                DropA["DropZone A<br/>存款明細"]
                DropB["DropZone B<br/>IP 紀錄"]
            end

            Control["ControlPanel<br/>控制面板"]

            subgraph ActionSection["動作區"]
                ExecBtn["ExecuteButton<br/>執行分析"]
                Progress["ProgressBar<br/>進度條"]
            end

            Warning["WarningBanner<br/>OpSec 警告"]
        end

        subgraph Output["輸出區"]
            Result["ResultTable<br/>結果預覽"]
            Download["DownloadButton<br/>下載報告"]
        end

        LogConsole["LogConsole<br/>日誌控制台"]
    end
...
flowchart LR
    subgraph Stores["Svelte Stores"]
        FS["filesStore<br/>檔案狀態"]
        SS["settingsStore<br/>設定狀態"]
        AS["analysisStore<br/>分析狀態"]
        LS["logsStore<br/>日誌狀態"]
    end
...
> 註：附件僅支援 **http/https 連結**（多行），不提供檔案上傳。
...
flowchart TB
    ...
    subgraph External["外部 crates"]
        calamine["calamine<br/>Excel 讀取"]
        xlsxwriter["rust_xlsxwriter<br/>Excel 寫入"]
        reqwest["reqwest<br/>HTTP 請求"]
        chrono["chrono<br/>時間處理"]
    end
...
flowchart TB
    subgraph Input["輸入階段"]
        U["使用者"] -->|拖曳| F1["檔案 A.xlsx"]
        U -->|拖曳| F2["檔案 B.xlsx"]
    end

    subgraph Frontend["前端處理"]
        F1 --> V1["驗證格式"]
        F2 --> V2["驗證格式"]
        V1 --> S1["更新 fileStore"]
        V2 --> S1
    ...
    subgraph Bridge["Tauri 橋接"]
        S1 -->|invoke load_file| CMD1["file_ops::load"]
        CMD1 -->|回傳| META["檔案元資料"]
    end

    subgraph RustParse["Rust 解析"]
    ...
    subgraph RustProcess["Rust 處理"]
        DF1 --> PP["processor::preprocess"]
        PP -->|隱藏敏感欄位| PP
        PP -->|收支分流| SPLIT["分離 income/expense"]
    end

    subgraph RustMatch["Rust 比對"]
        SPLIT --> M["matcher::match_ip"]
        DF2 --> M
        M -->|建立索引| IDX["HashMap&lt;account, Vec&lt;IpRecord&gt;&gt;"]
        M -->|時間窗口| WINDOW["[-1s, +2s] 篩選"]
        WINDOW --> RESULT["比對結果"]
    end

    subgraph RustWhois["Rust Whois"]
        RESULT -->|如果啟用| W["whois::query"]
...
```mermaid
flowchart TB
    subgraph Security["安全設計"]
        subgraph Offline["離線保護"]
            O1["本機運算"]
            O2["無雲端上傳"]
            O3["記憶體處理"]
        end

        subgraph Memory["記憶體安全"]
            M1["Rust 所有權"]
            M2["自動清理"]
            M3["無 GC 暫停"]
        end
