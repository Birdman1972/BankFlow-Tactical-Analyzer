...
#[tauri::command]
pub async fn load_file(path: String, state: State<'_, AppState>) -> Result<FileMetadata, String> {
    ...
    // Parse transactions
    let transactions = Parser::parse_transactions(&path_buf)
...
#[tauri::command]
pub async fn load_ip_file(path: String, state: State<'_, AppState>) -> Result<FileMetadata, String> {
    let path_buf = PathBuf::from(&path);
    ...
    // Parse IP records
    let ip_records = Parser::parse_ip_records(&path_buf)
