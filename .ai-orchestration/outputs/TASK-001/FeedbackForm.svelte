<script lang="ts">
  ...
  export let onSubmit: ((payload: FeedbackPayload) => Promise<void>) | null = null;

  const dispatch = createEventDispatcher<{ submit: FeedbackPayload }>();
  ...
  const status = $state<'idle' | 'success' | 'error'>('idle');
  const statusMessageKey = $state('');
...
<form class="cyber-panel p-4 space-y-4" on:submit={handleSubmit}>
